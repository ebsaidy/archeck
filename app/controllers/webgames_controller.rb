class WebgamesController < ApplicationController
	...
	def add_game
		...
		url = "http://165.227.68.43/games/add_game?name_of_game=#{name_of_game}&description=#{description}&no_of_balls=0&normal_ball=#{normal_ball}&power_ball=#{power_ball}&draw_day=#{draw_day}&status=1"
	...
	def index
		url = "http://165.227.68.43/games/get_games?"
	...
	def winning_balls
		...
		url = "http://165.227.68.43/games/winning_balls?game_name=#{game_name}&first_ball=#{first_ball}&second_ball=#{second_ball}&third_ball=#{third_ball}&fourth_ball=#{fourth_ball}&fifth_ball=#{fifth_ball}&power_ball=#{power_ball}&draw_date=#{draw_date}"
	...
	def get_balls
		...
		url = "http://165.227.68.43/games/get_balls"
		response = HTTParty.get(url).parsed_response.to_xml
	...
	# def get_games
	# 	url = "http://localhost:3000/games/get_game?"
	# 	response = HTTParty.get(url).parsed_response.to_xml
	# 	render xml: response, root: 'objects', children: 'object'
	# end
	def show
	...
	def edit
		url = "http://165.227.68.43/games/edit?id=#{params[:id]}"
		response = HTTParty.get(url).parsed_response.to_xml
		doc = Nokogiri::XML(response)
	...
	def edit_balls
		url = "http://165.227.68.43/games/edit_balls?id=#{params[:id]}"
		response = HTTParty.get(url).parsed_response.to_xml
		doc = Nokogiri::XML(response)
		doc.xpath("//hash").each do |node|
			@id = node.at('id').content.to_s
	...
	def update_balls
		...
		power_ball  = 	params[:power_ball]
		draw_date  = 	params[:draw_date]
		url = "http://165.227.68.43/games/update_balls?id=#{id}&game_name=#{game_name}&first_ball=#{first_ball}&second_ball=#{second_ball}&third_ball=#{third_ball}&fourth_ball=#{fourth_ball}&fifth_ball=#{fifth_ball}&power_ball=#{power_ball}&draw_date=#{draw_date}"
	...
	def update
		...
		power_ball   = 	params[:power_ball]
		draw_day     = 	params[:draw_day]
		url = "http://165.227.68.43/games/update?id=#{id}&name_of_game=#{name_of_game}&description=#{description}&no_of_balls=0&normal_ball=#{normal_ball}&power_ball=#{power_ball}&draw_day=#{draw_day}&status=1"
	...
	def destroy
		url = "http://165.227.68.43/games/destroy?id=#{params[:id]}"
