class WebticketsController < ApplicationController
	before_action :logged_in_user
	def index
		
		url = "http://165.227.68.43/tickets/get_tickets?"
	...
	def show
		url = "http://165.227.68.43/tickets/search?ticket_number=#{params[:ticket_number]}"
		response = HTTParty.get(url).parsed_response.to_xml
		doc = Nokogiri::XML(response)
		error = doc.xpath("//hash/result")
		if !error.empty?
			# flash[:danger] = "Keyword not found"
		else
		doc.xpath("//hash").each do |node|
			@name = node.at('name-of-game').content.to_s#.nil? ? 'Null' : node.at('name-of-game').content.to_s
			@firstBall = node.at('first-ball').content.to_s#.nil? ? 'Null' : node.at('first-ball').content.to_s
			@secondBall = node.at('second-ball').content.to_s#.nil? ? 'Null' : node.at('second-ball').content.to_s
			@thirdBall = node.at('third-ball').content.to_s#.nil? ? 'Null' : node.at('third-ball').content.to_s
			@fourthBall = node.at('fourth-ball').content.to_s#.nil? ? 'Null' : node.at('fourth-ball').content.to_s
			@fifthBall = node.at('fifth-ball').content.to_s#.nil? ? 'Null' : node.at('fifth-ball').content.to_s
			@fortuneBall = node.at('fortune-ball').content.to_s#.nil? ? 'Null' : node.at('fortune-ball').content.to_s
			@createdAt = node.at('created-at').content#.nil? ? 'Null' : node.at('created-at').content
			@user = node.at('user').content.to_s#.nil? ? 'Null' : node.at('user').content.to_s
			@ticketNumber = node.at('ticket-number').content.to_s#.nil? ? 'Null' : node.at('ticket-number').content.to_s
			# @error = node.at('result').content.to_s#.nil? ? 'Null' : node.at('result').content.to_s
